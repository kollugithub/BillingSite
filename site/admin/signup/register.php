...
//	$name=$pwd=$cpwd=$mail=$no='';
	...
	if(isset($_POST['name']) && isset($_POST['pwd']) && isset($_POST['cpwd']) && isset($_POST['mail']) && isset($_POST['no']) && isset($_POST['captcha'])) {
		...
		$query_check=mysql_query("SELECT * FROM staff_dup WHERE mail='$mail'");
		if(mysql_num_rows($query_check)==1) {
			$user=mysql_fetch_assoc($query_check);
			mysql_query("UPDATE staff_dup SET name='$name',password='$pwd',contact='$no',date='$time' WHERE mail='$mail'");
		}else {
			$query=mysql_query("INSERT into staff_dup VALUES('$name','$mail','$pwd','$no','$time')");
			$query_check=mysql_query("SELECT * FROM staff_dup WHERE mail='$mail'");
			$user=mysql_fetch_assoc($query_check);
		}

		require "../../staff/login/forgot_pwd/classes/class.phpmailer.php"; // include the class name
		$KEY = "love@1612"; //key should be long and secret
		
		$time = time();
		$hash = md5( $user['mail'] . $time . $KEY);
		$url = "localhost/site/staff/signup/verify_deactivate.php?id=".$user['mail'].'&timestamp='.$time.'&hash='.$hash;
		...
		$pmail->Subject = "XStore Staff verify account";
