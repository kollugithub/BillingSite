...
if ($hash == md5( $user_id . $timestamp . $KEY )) {
    if ( time() - $timestamp > 1296000 ) { // 15 days
		...
		$query=mysql_query("SELECT * FROM customer_dup WHERE mail='$user_id'");
		if(mysql_num_rows($query)!=0) {
			...
			$pwd=$user['password'];
			$no=$user['contact'];	
			mysql_query("INSERT INTO customer VALUES('$name','$mail','$pwd','$no','0')");
			mysql_query("DELETE FROM customer_dup WHERE mail='$user_id'");
			header('Location: ../login/index.php');
		}else {
			$query=mysql_query("SELECT * FROM customer WHERE mail='$user_id'");
			if(mysql_num_rows($query)!=0) {
