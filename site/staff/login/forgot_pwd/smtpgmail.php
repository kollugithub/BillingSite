...
if(isset($_POST['mail'])) {
	...
	$email=$_POST['mail'];
	if(!empty($email)) {
		$query=mysql_query("SELECT mail FROM staff WHERE mail='$email'");
		if(mysql_num_rows($query)==1) {
			...
			$url = "localhost/site/staff/login/forgot_pwd/resetpassword.php?id=".$user['mail'].'&timestamp='.$time.'&hash='.$hash;
		...
		$mail->Subject = "XStore Staff Reset Password";
